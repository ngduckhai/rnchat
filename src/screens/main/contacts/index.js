...
class Contacts extends PureComponent {
  ...
  constructor(props) {
    super(props);
    this.isGroupDetails = this.props.navigation.getParam(
      'isGroupDetails',
      false,
    );
  ...
  _renderSelectedUser = ({item}) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => this.toggleUserSelect(item)}>
  ...
  searchUsers = () => {
    ...
    if (str.length > 2) {
      ...
      showAlert('Nhập ít nhất 3 kí tự!');
    }
  };
  ...
  goToCreateDialogScreen = () => {
    ...
    if (this.isGroupDetails) {
      const addParticipant = this.props.navigation.getParam(
        'addParticipant',
        false,
      );
      navigation.goBack();
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
          ...
          {!this.isGroupDetails && (
            <TouchableOpacity
              ...
              <Text style={styles.dialogTypeText}>
                {dialogType ? `Trò chuyện nhóm` : `Trò chuyện riêng tư`}
              </Text>
            </TouchableOpacity>
          )}
        </View>
        <View
          style={this.selectedUsers.length > 0 && styles.containerCeletedUsers}>
          <FlatList
        ...
        {this.userNotFound ? (
          <Text style={styles.userNotFound}>Không tìm thấy người dùng</Text>
        ) : (
          <View style={{flex: 1}}>
            <FlatList
              data={this.listUsers}
        ...
        {this.selectedUsers.length > 0 && (
          <CreateBtn
            goToScreen={this.goToCreateDialogScreen}
